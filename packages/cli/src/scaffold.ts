	...
	return {
		...
		agents: {
			assistant: {
				profile: "claude",
				instructions:
					"You are a careful assistant. Inspect the repo at your cwd read-only to ground your answer. You write nothing.",
...
function loopManifest(): unknown {
	return {
		description:
			"Build a change, review it with a structured verdict, and verify with a check -- repeat until the check passes AND the reviewer approves (sandboxed).",
		input: "task",
		agents: {
			builder: {
				profile: "claude",
				instructions:
					"You implement a small, well-scoped change and keep going until the check passes and the reviewer approves.",
				filesystem: "read-write",
			},
			reviewer: {
				profile: "codex",
				instructions:
					"You review the diff for correctness and scope and return a structured verdict. You do not edit files.",
				filesystem: "read-only",
			},
		},
		steps: [
...
function manifestFor(template: Template): unknown {
...
export function scaffoldRoutine(cwd: string, name: string, template: Template): ScaffoldResult {
	...
	const manifest = manifestFor(template) as { description?: string };
	...
	const profileRegistry = config.profiles;
