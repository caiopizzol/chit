...
2. Read the emitted plan JSON. It is a native sequential plan: `schema` 1, a `title`, optional `id`/`baseBranch`/`apply`/`cleanup`, and `steps` carrying `id`/`title`/`body`/`dependsOn`/`requiredChecks` and the optional `commitMessage`/`manifestPath`/`maxIterations`/`callTimeoutMs`. `dependsOn` is a code dependency, not a launch gate, and there is no batch task graph and no invented field. `commitMessage` is the single-line conventional commit subject the gated apply commits that step under on the integration branch (omitted, the commit falls back to `plan step <id>: <title>`); it is part of the reviewed plan, so the approval hash binds it.
