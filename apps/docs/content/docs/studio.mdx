...
The Loops drawer is read-only. It lists loop runs read from `.chit/loops` under the cwd, newest first, and lets you open one loop's iterations. Each iteration links to its recorded transcript through a `detailsRef`, which opens the [audit view](/docs/audit-log) for that run.
