...
import { BatchStore, BatchStoreError, batchesDir } from "./store.ts";
...
describe("BatchStore", () => {
	test("lives under the state dir keyed by repo, not in the repo", () => {
		expect(batchesDir(cwd).startsWith(stateDir)).toBe(true);
		store.create(batch("c1"));
		expect(store.get("c1")).toMatchObject({ id: "c1", status: "running" });
	});

	test("create refuses to clobber an existing batch", () => {
		store.create(batch("c1"));
		expect(() => store.create(batch("c1"))).toThrow(/already exists/);
	});
	...
	test("update on a missing batch throws", () => {
		expect(() => store.update("ghost", (c) => c)).toThrow(/no batch/);
	});

	test("list is newest-first and skips corrupt files", () => {
		store.create(batch("old", { createdAt: "2026-06-01T10:00:00.000Z" }));
		store.create(batch("new", { createdAt: "2026-06-01T11:00:00.000Z" }));
		writeFileSync(join(batchesDir(cwd), "broken.json"), "not json");
		expect(store.list().map((c) => c.id)).toEqual(["new", "old"]);
	});

	test("rejects an unsafe batch id", () => {
		expect(() => store.get("../evil")).toThrow(BatchStoreError);
	});
});
