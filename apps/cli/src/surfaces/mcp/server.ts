...
// The real manifest-binding resolver the plan/batch gates and engines share: fresh
// config per call (the worker's own lifecycle, so config drift surfaces as binding
// drift instead of being pinned away), git object reads via realGit. Throws
// ManifestBindingError for every failure so callers treat "cannot resolve" as drift.
const resolveManifestBindingReal: ResolveManifestBinding = (p) => {
...
function launchConvergeJob(p: {
	...
	// The accepted upstream handoffs injected into this run's task at launch (Phase 4), body-free.
	// Persisted on the job so trace/status/audit report what the step consumed; the bodies live in
	// `task`. Only a consuming plan step sets it.
	...
	const job: LoopJobRecord = {
		...
		...(p.recipe !== undefined && { recipe: p.recipe }),
		...(p.planHandoffs !== undefined && { planHandoffs: p.planHandoffs }),
...
export function backgroundLoopTraceResponse(runId: string, job: LoopJobRecord, raw: LoopRecord[]) {
	return {
		...
		...(job.participants !== undefined && { participants: job.participants }),
		// The body-free upstream handoffs this run consumed at launch (Phase 4 dependent injection),
		// persisted on the job record so the run-detail trace says exactly what data fed this step --
		// alias + producing step + handoff id + accepted digest + bytes, never the body (the bodies live
		// in the task brief and the producing step's accepted record). Omitted on a run that consumed
		// nothing and on legacy records.
		...(job.consumedHandoffs !== undefined && { consumedHandoffs: job.consumedHandoffs }),
...
const planDeps: PlanEngineDeps = {
	...
	// Producer handoff capture's filesystem touch + trust boundary (containment, regular-file, byte
	// cap, strict UTF-8 decode, and byte digest); the pure capture layer adds JSON parseability +
	// status projection on top of what it returns.
	readHandoffFile: (worktreePath, relPath, maxBytes) =>
		readHandoffFileReal(worktreePath, relPath, maxBytes),
	now: () => Date.now(),
};
...
function planError(e: unknown) {
	if (
		// The approval-gate refusal is the caller's own and carries no local path, so its
		// message passes through verbatim, like a parse/engine/store/worktree error.
		e instanceof PlanApprovalRefused ||
	...
	"chit_plan_start",
	{
		...
		inputSchema: {
			...
			max_iterations: z
				...
				.optional()
				.describe(
					"Per-step iteration budget when a step declares none. Default 3. Bound into the approval hash, so it cannot be changed between dry run and confirm.",
				),
			confirm: z
				.boolean()
				.optional()
				.describe(
					"Omit or false for a dry run (parse, resolve base, return the plan/base/approvalHash, create nothing). true to start, which requires a matching approval_hash.",
				),
			approval_hash: z
				.string()
				.optional()
				.describe(
					"The approvalHash from a prior chit_plan_start dry run of this exact plan, base, and max_iterations. Required when confirm is true; the start is refused if it does not match the recomputed hash.",
				),
		},
	},
	async ({ plan, plan_path, cwd, base_branch, max_iterations, confirm, approval_hash }) => {
		try {
			const { store, cwd: runCwd } = planStoreFor(cwd);
			// runPlanStart owns the parse/gate/hash/start glue (unit-tested with fake deps); the
			// handler only adds the real store + deps and the main-repo resolution.
			...
			// Dry run: the plan + base + hash to approve. Nothing was launched and no plan record,
			// worktree, job, or branch was created.
			if (!result.launched) {
				return jsonResult({
					launched: false,
					strategy: result.strategy,
...
server.registerTool(
	...
	{
		description:
			"Advance a plan, OR apply a review_ready step. Without an `apply` payload: reconcile the running step's finished job into its state (converged -> review_ready; completed-but-not-converged -> needs_human; a vanished/stale job or a failed run -> failed), then launch the next runnable step. With an `apply` payload: run the gated apply for that review_ready step -- flow its worktree diff into the plan integration branch, commit it as a step commit (under the step's reviewed commitMessage from the approved plan, else `plan step <id>: <title>`), advance the tip, and mark the step applied (only then can a dependent launch, cut from the new tip). Apply is DRY-RUN by default (reports what would land, mutates nothing); pass apply.confirm=true to apply + commit. A conflict refuses the whole apply (nothing committed, the step stays review_ready); untracked files are applied only when named in apply.include_untracked. When the step declared handoffs, the apply response carries each captured handoff with its full body (apply is the gate where you inspect the exact content being accepted) -- a confirmed apply freezes them into immutable accepted plan state in the same commit; if a handoff's content changed since the producing step's reviewer saw it the confirm REFUSES and re-reports the new digest. Apply does NOT also launch the next step -- call advance again (no payload) to launch it. This is the only progression trigger besides start.",
		inputSchema: {
			plan_id: z.string().describe("The plan id, from chit_plan_start or chit_plan_list."),
			cwd: z
				.string()
