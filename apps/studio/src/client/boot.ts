// SSR boot consumption. The server inlines the launch token under window.__chit.
// This module:
...
// Pure: store the token. Testable without a DOM.
export function processBootPayload(payload: BootInput, storage: TokenStorage): void {
...
export function consumeBoot(): void {
	const w = window.__chit;
	if (!w || typeof w.token !== "string") {
		throw new Error("chit studio: window.__chit missing or incomplete");
	}
	processBootPayload({ token: w.token }, sessionStorage);
