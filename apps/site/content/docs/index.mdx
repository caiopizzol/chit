...
- **Foreground.** Checkpoint every iteration. chit runs one round at a time (`chit_converge_start` / `chit_converge_next`); you read the diff and the verdict, then advance.
- **Background job.** Run one task unattended. `chit_converge_run` converges in a detached worker against a git worktree; check on it with `chit_job_status` and read the receipt.
