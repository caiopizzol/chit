...
Studio opens as a control tower: a calm, read-only view of what is alive across Chit right now. Foreground loops running in your sessions and durable background jobs show up side by side, with the selected run's detail and a small console of recent transitions. It reads the runtime; it does not drive it.
...
Run actions. The tower observes; it does not apply, cancel, or clean up runs. Launching a run from Studio and streaming its output live are not wired up.
