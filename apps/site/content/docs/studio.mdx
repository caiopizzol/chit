...
- The inspector edits safe fields directly: manifest `description`, participant `role`, `session`, and `permissions.filesystem`, plus the call `prompt` and format `format` templates.
