---
title: Audit log
description: The durable, append-only record of a run, including prompts, outputs, usage, and timing
---
...
- `chit converge` audits by default. Each iteration's run is recorded, and the loop iteration links to it through the loop log's `auditRef`.
- `chit run --audit` records one run. The run id is printed on stderr.
- The MCP surface records a run when `chit_start` is called with `audit: true`, and every background run is audited. Each audited run (or loop iteration) gets an `audit_ref` you read back with `chit_audit_show` (a one-shot run's `audit_ref` equals its `run_id`; a loop has one per iteration, surfaced in `chit_trace`).
...
`list` prints the runs, newest first, with status and a usage summary. `show` prints one run's recorded participant config, then its timeline and a usage summary. Both take `--json`. `show` takes `--blobs` (alias `--include-bodies`) to print the blob bodies (rendered prompts, outputs, and raw adapter event bodies); without it, only the event timeline is shown.
...
## Rolling up across runs

`list` and `show` answer "what happened in one run". `stats` rolls the same receipts into one metrics-only summary across runs:

```text
chit audit stats
chit audit stats --surface converge --since 2026-06-01 --limit 50
chit audit stats --all-repos
```

It reports how many recent runs were considered (and how many had an unreadable or empty log), the breakdown by surface and by run status (so convergence vs failure, blocked, and timeout incidence are visible), the recipe breakdown when runs declared one, the convergence verdict/decision counts with total findings, how many iterations named a real verification source, and summed token usage and reported cost. `--json` emits the structured aggregate.

The convergence verdict, decision, and iteration counts are derived from the audited loop-iteration events recorded for each run. That event is a newer addition to the audit record, so receipts written before it existed carry no loop-iteration events and those older runs report zero convergence iterations here even when they did converge. This is expected, not a data-loss bug.

The audit store is one per-user state dir shared by every repo, so `stats` scopes to the current repo by default: it keeps only runs whose recorded working directory resolves to this repo's root (a run from any subdirectory counts). `--all-repos` folds runs from every repo instead. The repo match is an internal filter; the path is never printed.

It reads only the receipt metadata, never a blob body, and emits only counts, low-cardinality enums (surface, status, verdict, recipe id), and numeric sums. It never prints prompts, outputs, file paths, error text, or `scope` (which is accepted only as a filter via `--scope`).
...
- Do not commit it, and do not share a transcript without reading what is in it. `chit audit show <runId> --blobs` prints exactly what would be shared.
- Plain `chit run` and MCP runs do not audit unless you ask. `chit converge` audits by default because the loop log already points at the transcript.
