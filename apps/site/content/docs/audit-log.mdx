...
- `chit converge` audits by default. Each iteration's run is recorded, and the loop iteration links to it through the loop log's `detailsRef`.
- `chit run --audit` records one run. The run id is printed on stderr.
- The MCP surface records a run when `chit_start` is called with `audit: true`. The audit run reuses the MCP `run_id`.
