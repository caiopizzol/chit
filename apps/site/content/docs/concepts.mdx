...
An agent is an invocable participant: Codex CLI, Claude CLI, or anything else reachable through an adapter. Agents live in a registry inside your config (`~/.config/chit/config.json`) with built-in `codex` and `claude` entries that your config merges on top. A manifest never talks to a vendor directly; it names a participant, and the registry resolves it to an adapter.
