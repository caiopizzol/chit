...
- **Config: agents and roles.** Config is layered: built-in `codex` and `claude`, then `~/.config/chit/config.json` (global), then the repo's `chit.config.json`. Both files hold `agents` (model, reasoning effort, and timeouts on named agents; built-in ids cannot be redefined) and `roles` (reusable participant profiles a manifest references by id). A later layer replaces an agent or role by id, whole. The repo file cannot set `env` or `strictMcp`; those belong in the global config. A manifest references agents or roles. `chit show` and `chit audit show` report the effective config, and `chit doctor` reports which layer defined each agent.
