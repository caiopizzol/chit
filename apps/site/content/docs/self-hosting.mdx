...
- **Config: agents, roles, and execution profiles.** `~/.config/chit/config.json` holds `agents` (model, reasoning effort, and timeouts on named agents, merged over built-in `codex` and `claude`), `roles` (reusable participant profiles a manifest references by id), and optional `profiles` (a closed menu of vetted execution profiles for planner-authored drafts). A manifest references agents or roles. A draft selects a profile by id. `chit show` and `chit audit show` report the effective config so you can see what a run used.
