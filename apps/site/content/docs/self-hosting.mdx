...
3. Inspect what happened: `chit_trace` (or read the loop log), and open a transcript with `chit_audit_show` / `chit audit show <runId>` (the run id is on the iteration's `auditRef`). The transcript carries the prompts, outputs, live adapter events, usage, and the recorded per-participant config.
