...
- **Config: agents and roles.** `~/.config/chit/config.json` holds `agents` (model, reasoning effort, and timeouts on named agents, merged over built-in `codex` and `claude`) and `roles` (reusable participant profiles a manifest references by id). A manifest references agents or roles. `chit show` and `chit audit show` report the effective config so you can see what a run used.
