...
- **Config: agents and roles.** `~/.config/chit/config.json` holds the `agents` section (model, reasoning effort, and timeouts on named agents, merged over built-in `codex` and `claude`) and the `roles` section (reusable participant profiles, above). A manifest references either by id. `chit show` and `chit audit show` report the effective config so you can see what a run used.
