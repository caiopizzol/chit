...
List audited runs, newest first. Input: `limit?`. Returns `{ runs[] }`, each run `{ run_id, manifestId, surface, scope?, iteration?, startedAt?, status, stepCount, usage?, openCall? }`, where `status` is the `run.completed` status or `incomplete`, and `openCall` (when present) names a step whose adapter call started but never completed (killed mid-call).

### chit_audit_show

Show one run. Inputs: `run_id`, `include_bodies` (default `false`), `verbose` (default `false`). Returns `{ summary, incompleteReason?, participants?, timeline[], note? }`: the summary above, the reason when incomplete, the participant config recorded at start, and the structured event timeline. Without `verbose` the timeline is a receipt (the raw per-call adapter events are hidden, and `note` says how many). Prompt/output/event bodies attach to their timeline entries (`input`/`output`/`raw`) only when `include_bodies` is true, since they can be large or hold secrets.
