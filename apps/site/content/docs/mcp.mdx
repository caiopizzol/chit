...
Returns the run view with the initial ready set (one-shot) or the open loop (loop). Errors: unreadable or invalid manifest, a one-shot manifest given a `task`, a loop run with no `scope`, unknown agent, enforcement gap without the flag, `required_checks` on a one-shot run.
...
- With a `plan_id`: waits until the active step's job needs reconciliation, the plan is `ready_for_apply`, or the plan is terminal. It does **not** advance, reconcile, apply, or launch. The operator loop is: `chit_wait` -> `chit_plan_advance` -> `chit_plan_status`, repeated until a step is ready to apply or the plan settles.
...
Approve and start a plan. The start is **gated**: with `confirm` omitted or false it parses the plan, resolves the base to a concrete commit SHA, returns the normalized plan plus `approval_hash`, and creates nothing. With `confirm:true`, chit re-parses, re-resolves, recomputes the hash over the plan, resolved base, and `max_iterations`, and refuses unless `approval_hash` matches. On a match it launches the first runnable step from the approved commit SHA.

Inputs: `plan` (inline object or JSON string) or `plan_path`, `cwd?`, `base_branch?`, `max_iterations?`, `confirm?`, `approval_hash?`.
...
Retire the plan-managed worktrees and branches once every step is applied or the plan is cancelled. Safe by default: with `confirm` omitted or false it lists what would be removed and removes nothing. It keeps plan, job, loop, and audit receipts. Inputs: `plan_id`, `cwd?`, `confirm?`.
...
- **`confirm: true` re-plans and verifies the hash.** chit re-normalizes the graph, re-resolves the base, and recomputes the hash over the task graph, the resolved base commit, and the launch knobs (`max_parallel`, `max_iterations`, `manifest_path`, `required_checks`, `call_timeout_ms`). It **refuses unless `approval_hash` matches**, so a task, base, or knob edited after the dry run cannot start on a stale hash. On a match it launches the initial runnable wave (no-dependency tasks, up to `max_parallel`) from the approved commit SHA -- pinned, even if the ref has since moved -- and returns the `batch_id` and batch view.
