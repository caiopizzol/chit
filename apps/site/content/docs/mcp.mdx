...
- `recipe?` - a vetted config recipe id from `chit_recipes`. Recipes are converge-only today: the recipe supplies the loop manifest plus default `max_iterations` / `call_timeout_ms`, and the run still needs `task` and `scope`. Mutually exclusive with `manifest_path`; explicit `max_iterations` and `call_timeout_ms` inputs override the recipe defaults.
