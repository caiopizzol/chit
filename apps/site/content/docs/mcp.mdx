...
It never applies a `review_ready` step, confirms an apply, cleans up, cancels, or approves anything. Use it after `chit_plan_start`, then again after you explicitly apply a `review_ready` step with `chit_plan_advance`.
