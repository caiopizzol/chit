...
2. The chat calls the advisor with `chit_start` using a stable `scope` per thread, `cwd` set to the repo, and inputs that describe the task plus what changed, then runs the review step with `chit_run_step`.
