...
- Reuse one `scope` per thread; pass `cwd` set to the repo; do not edit the advisor `instructions` mid-thread (they are in the session fingerprint, so a change forks a fresh thread).
