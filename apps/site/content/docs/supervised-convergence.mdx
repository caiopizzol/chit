...
- chit's implementer is real. `claude-cli` runs `claude --print`, which uses tools and edits files, and the `converge` manifest (`examples/converge.json`) drives it autonomously (a write-capable Claude implements, a read-only `codex-exec --sandbox read-only` reviews). So the executor can be either the interactive Claude Code chat (full conversational context, your per-edit approval) or a chit-spawned Claude (autonomous, run under a worktree plus a human checkpoint).
