...
chit drives the loop with its converge tools: `chit_converge_start` opens the loop, then each `chit_converge_next` runs one iteration where the implementer makes the slice and the reviewer checks the diff. By default the implementer is a write-capable Claude and the reviewer is a read-only Codex, but the roles are assigned in the chit, not fixed to a vendor (see [self-hosting](/docs/self-hosting)). Each iteration is recorded; `chit_audit_list` / `chit_audit_show` read the transcript back. You see what ran, what the reviewer said, and the token usage. No copy-paste between terminals.

This first run is in **foreground** mode: one iteration per call, with you reading each diff. The same loop also runs **unattended** as a background job (`chit_converge_run`), and several tasks at once as a **batch** (one git worktree each, with dependencies). See [self-hosting](/docs/self-hosting) and the [MCP surface](/docs/mcp).

The implementer edits files, so run this against a git worktree, not your main checkout. For a read-only fan-out instead of the implement/review loop, run a manifest like `examples/consult.json` with `chit_run_start` from a source checkout.
