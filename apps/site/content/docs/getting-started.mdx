...
This first run is in **foreground** mode: one iteration per `chit_next`, with you reading each diff, holding one `run_id` the whole way. The same run goes **unattended** with `mode: "background"` (a detached worker drives it to completion; poll with `chit_status`, stop with `chit_cancel`), and several tasks run at once as a **batch** (one git worktree each, with dependencies). See the [MCP surface](/docs/mcp).
