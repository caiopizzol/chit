...
| `participants` | yes      | Inline aliases over registry agents, or references to a config role, with instructions, session, and permissions. |
| `steps`        | yes      | Map of step id to step body. Step types: `call`, `format`.            |
| `output`       | yes      | Step id whose output is the final return value.                       |

A participant is either inline or a reference to a named role. An inline participant declares `agent`, `instructions`, and `session` directly. A role reference names a role from config and inherits those fields, restating only what it overrides.

Fields:

- `role`: id of a role defined in the `roles` section of `~/.config/chit/config.json`. Optional, and the alternative to declaring `agent`/`instructions`/`session` inline. When present, the participant inherits the role's `agent`, `instructions`, `session`, and `permissions`; any of those the participant also sets replaces the role's value (a shallow replace, not a merge).
- `agent`: id from the agent registry. Required on an inline participant; an optional override on a role reference. A role with no default agent is model-agnostic, and a participant referencing it must supply one.
...
- `session`: `stateless` | `per_topology` | `per_scope`. `per_scope` means the session persists across runs sharing a scope (e.g. the same Claude session plus worktree). When any participant uses `per_scope`, the runtime infers `can_provide_stable_scope` as a surface requirement. A `per_scope` session inherited from a role counts the same as one declared inline.
- `permissions.filesystem`: `read_only` | `write`. Optional. Default is `read_only`. Only declare it when the participant needs `write`.

A role reference is the inline form with the shared fields lifted into config:

```json
"participants": {
  "reviewer": { "role": "reviewer" },
  "implementer": { "role": "reviewer", "agent": "claude", "permissions": { "filesystem": "write" } }
}
```

Both participants inherit the `reviewer` role's instructions and session. `implementer` overrides the agent and grants write. Role references resolve against config when the chit runs, not when it parses: an unknown role, or a model-agnostic role used without a participant `agent`, is a resolution error reported at run time, not a parse error.

Inline participants still let the same registry agent participate twice with different instructions. Without an inline alias or a role reference, a participant would be pure renaming and should not exist.
...
An optional top-level `policy` decides what a run *does*. Absent, it defaults to one-shot, so every existing manifest is unchanged.
...
- **one-shot** (the default) runs the step DAG once.
- **loop** is the converge pattern: an implementer step and a reviewer step run in turn, and chit reads the reviewer's `proceed | revise | block` verdict each round. `implementStep` and `reviewStep` name the call steps; `maxIterations` is the budget (default 3).
...
```jsonc
{
  "policy": {
    ...
    "reviewStep": "review",
    "maxIterations": 3,
    // chit runs these ITSELF after a proceed review (ground truth, not the reviewer's
    // word). All pass -> converged; any fails -> revise; one cannot run -> needs-decision.
    "requiredChecks": [
...
`requiredChecks` is **structured, not a shell string**: `{ command, args?, name?, timeoutMs? }`, spawned as argv with no shell (a metacharacter in an arg is a literal argument). `command` is required; `args` defaults to `[]`; `name` is a display label; `timeoutMs` defaults to 120000. No `env`, `cwd`, or shell strings -- this is chit-executed verification, not a CI language. When declared, chit's result is authoritative over the reviewer's self-reported checks. A run can override these per invocation (`chit_start`'s `required_checks`, or a batch's task/batch-level checks) without editing the manifest; see the [MCP reference](/docs/mcp#chit-executed-verification).
...
7. **Config location.** Layered: `~/.config/chit/config.json` (global), then `chit.config.json` at the repo root. Both hold `agents` (the registry, alongside the non-redefinable built-in `codex` and `claude`) and `roles` (reusable participant profiles a manifest references by id). A later layer replaces an entry by id, whole; the repo file cannot set `env` or `strictMcp`. There is no separate `agents.json`. See [Config layering](/docs/concepts#config-layering).
