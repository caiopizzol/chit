...
7. **Config location.** `~/.config/chit/config.json`, holding `agents` (the registry, merged over built-in `codex` and `claude`), `roles` (reusable participant profiles a manifest references by id), and optional `profiles` (vetted execution profiles a planner-authored draft selects by id). There is no separate `agents.json`.
