...
A chit is a JSON manifest. This page is the reference for what it can declare. The two manifests in `apps/cli/examples/` are the canonical examples, and `src/manifest/parse.ts` is the source of truth for behavior. Sections marked **Deferred** describe decisions held back on purpose, with the rationale recorded inline.
...
- `codex-exec` enforces `filesystem: read_only` via `--sandbox read-only`.
- `claude-cli` does not currently enforce filesystem; it depends on Claude's own tool permissions.
- A generic subprocess adapter cannot enforce; install fails when `read_only` is requested unless the flag is set.

In practice: any manifest using a `claude-cli` participant requires `--allow-unenforced-permissions` today. Codex-only manifests run cleanly. When claude-cli is taught to restrict tools, the gap closes without manifest changes.
