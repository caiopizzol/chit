...
7. **Config location.** `~/.config/chit/config.json`, holding `agents` (the registry, merged over built-in `codex` and `claude`) and `roles` (reusable participant profiles a manifest references by id). There is no separate `agents.json`.
