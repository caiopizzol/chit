...
```txt
packages/cli/src/config.ts       config parser and profile validation
packages/cli/src/manifest.ts     routine parser and derived behavior
packages/cli/src/run.ts          text execution
packages/cli/src/converge.ts     loop execution
packages/cli/src/converge-run.ts sandbox orchestration
packages/cli/src/flow.ts         composed routines
packages/cli/src/adapter.ts      model-call seam and CLI adapters
packages/cli/src/doctor.ts       environment readiness checks
packages/cli/src/store.ts        receipts and stored patches
packages/cli/src/views.ts        terminal rendering
packages/cli/scenarios           executable scenario matrix
apps/site                        concise docs site
```
