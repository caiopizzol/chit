...
- *Migrated.* Repo directory (`/personal/handoff` → `/personal/chit`), root README, workspace package names (`chit-workspace`, `chit-cli`, `chit-web`, `@chit/core`), CLI binary name (`chit`), CLI help and stderr prefixes, landing site.
- *Not yet migrated (deliberately deferred).* State paths (`~/.config/handoff/agents.json`, `~/.local/state/handoff/sessions/`), install marker filename (`.handoff-install.json`), a handful of code comments. Changing these breaks existing local installs and the few tests that assert against them. The plan is one coordinated commit at the v0 cut.
...
**Status.** Pre-v0. Shipped: the runtime, CLI, Claude Code skill surface, MCP stepwise surface, inspector (ASCII / JSON / Mermaid / HTML), Studio (graph editor + read-only Loops view + audit transcript view), the convergence log, the supervised and autonomous implement/check loops, the audit log (full prompt/output transcripts on all three run surfaces, with retention, readable via `chit audit` and Studio), preservation of Codex's observable event stream as audit events on audited runs, the install marker, and safe lifecycle. Not shipped: Claude's event stream (still final-JSON mode, not stream-json) and declared human-checkpoint or loop steps inside manifests. Event capture is preserved after the call completes, not live, and never hidden model reasoning. Brand copy must match what is shipped. Do not promise what is not.
