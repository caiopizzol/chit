...
- *Migrated at the v0 cut.* State and config paths (`~/.config/chit/agents.json`, `~/.local/state/chit/sessions/`, `~/.local/state/chit/audit/`) and the install marker (`.chit-install.json`). The legacy `handoff` paths and `.handoff-install.json` are still READ as a fallback for one release, so existing local installs keep working; new writes use the `chit` names. Historical artifacts (`proposals/`, receipts, `dogfood/`) keep the `handoff` name on purpose: they record when the project was called handoff.
