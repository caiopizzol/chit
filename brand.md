...
- *Migrated at the v0 cut.* State and config paths (`~/.config/chit/config.json`, `~/.local/state/chit/sessions/`, `~/.local/state/chit/audit/`) and the install marker (`.chit-install.json`). chit reads and writes only the `chit` names; the old `handoff` paths and `.handoff-install.json` are no longer read, so local state from before the cut must be moved to the `chit` paths to stay visible. Some historical notes keep the `handoff` name on purpose: they record when the project was called handoff.
